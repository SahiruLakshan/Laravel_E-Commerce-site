<nav class="navbar navbar-expand-lg navbar-light" style="background-color:red">
  <div class="container-fluid">
    <a class="navbar-brand" href="{{url('/')}}" style="text-decoration: none;"><h4>Tec Com</h4></a>
    ...
    <div class="collapse navbar-collapse" id="navbarSupportedContent">
      <ul class="navbar-nav me-auto mb-2 mb-lg-0">
        ...
        <li class="nav-item">
          <a class="nav-link" href="{{url('offer')}}" style="text-decoration: none;"><b>Special Offer</b></a>
        </li>
      ...
      <div>
        <form action="{{url('searchproduct')}}" method="POST">
          ...
          <div class="input-group">
            <input class="form-control" style="width: 10cm" id="search-product" name="product_name" required type="search" placeholder="Search Products Model Name" aria-label="Search">
      ...
      <ul class="navbar-nav ms-auto">
                        ...
                        @guest
                            @if (Route::has('login'))
                                <li class="nav-item" >
                                    <a class="nav-link" href="{{ route('login') }}" style="text-decoration: none;"><b>{{ __('Login') }}</b></a>
                                </li>
                            @endif

                            @if (Route::has('register'))
                                <li class="nav-item">
                                    <a class="nav-link" href="{{ route('register') }}" style="text-decoration: none;"><b>{{ __('Register') }}</b></a>
                                </li>
                            @endif
                        @else
                        <li class="nav-item dropdown">
                            <a class="nav-link dropdown-toggle" href="#" id="navbarDropdown" role="button" data-bs-toggle="dropdown" aria-expanded="false" style="text-decoration: none;">
                              <img src="{{ asset('uploads/profile/' .Auth::user()->image) }}" class="rounded-circle" width="35px" height="35px">
                              <span style="padding-left:10px"><b>{{ Auth::user()->name }} </b></span>
                            </a>
